...
@NamedQueries({
    ...
    , @NamedQuery(name = "Pagamento.findByPagamentoValortotal", query = "SELECT p FROM Pagamento p WHERE p.pagamentoValortotal = :pagamentoValortotal")
    , @NamedQuery(name = "Pagamento.findByPagamentoTarifa", query = "SELECT p FROM Pagamento p WHERE p.pagamentoTarifa = :pagamentoTarifa")
    , @NamedQuery(name = "Pagamento.findByPagamentoData", query = "SELECT p FROM Pagamento p WHERE p.pagamentoData = :pagamentoData")
    , @NamedQuery(name = "Pagamento.findByPagamentoTipo", query = "SELECT p FROM Pagamento p WHERE p.pagamentoTipo = :pagamentoTipo")
    , @NamedQuery(name = "Pagamento.findByPagamentoDescricao", query = "SELECT p FROM Pagamento p WHERE p.pagamentoDescricao = :pagamentoDescricao")
    , @NamedQuery(name = "Pagamento.findByPagamentoFlag", query = "SELECT p FROM Pagamento p WHERE p.pagamentoFlag = :pagamentoFlag")})
public class Pagamento implements Serializable {
    ...
    @Column(name = "pagamento_valortotal")
    private Double pagamentoValortotal;
    @Column(name = "pagamento_tarifa")
    ...
    @Size(max = 45)
    @Column(name = "pagamento_flag")
    private String pagamentoFlag;
    @JoinColumn(name = "conta_conta_id", referencedColumnName = "conta_id")
    @ManyToOne(optional = false)
    private Conta contaContaId;
    @JoinColumn(name = "pagamento_status_pagamento_status", referencedColumnName = "pagamento_status")
    @ManyToOne(optional = false)
    private PagamentoStatus pagamentoStatusPagamentoStatus;
    ...
    public PagamentoStatus getPagamentoStatusPagamentoStatus() {
        return pagamentoStatusPagamentoStatus;
    }

    public void setPagamentoStatusPagamentoStatus(PagamentoStatus pagamentoStatusPagamentoStatus) {
        this.pagamentoStatusPagamentoStatus = pagamentoStatusPagamentoStatus;
    }
    ...
    public String toString() {
        return "br.com.jsn.entidade.Pagamento[ pagamentoId=" + pagamentoId + " ]";
    }
